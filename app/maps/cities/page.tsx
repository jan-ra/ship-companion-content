...
export default function CitiesPage() {
  ...
  const addCity = () => {
    ...
    toast.success(t("cities.toastCityAdded"));
  };

  const removeCity = (id: number) => {
    const pointsUsingCity = points.filter((p) => p.cityId === id);
    if (pointsUsingCity.length > 0) {
      toast.error(t("cities.cannotDeleteWithPOI", { count: pointsUsingCity.length }));
    ...
    <div className="p-6 max-w-6xl mx-auto">
      <div className="mb-6">
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-3xl font-bold mb-2">{t("cities.title")}</h1>
            <p className="text-muted-foreground">
              {t("cities.subtitle", { count: cities.length })}
            </p>
          </div>
          {devMode && (
      ...
      {!devMode && (
        <Alert className="mb-6">
          ...
          <AlertTitle>{t("cities.devAccessRequired")}</AlertTitle>
          <AlertDescription>
            {t("cities.devAccessDesc")}
          </AlertDescription>
        </Alert>
      )}
        ...
        <div className="lg:col-span-2">
          <Card>
            <CardHeader className="pb-3">
              <div className="flex items-center justify-between">
                <CardTitle className="text-lg">{t("cities.allCities")}</CardTitle>
              ...
              <div className="space-y-6">
                ...
                {devMode && (
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div className="space-y-2">
                      <Label>{t("cities.isIslandLabel")}</Label>
                      <div className="flex items-center space-x-2 pt-2">
                        ...
                        <Label className="font-normal">
                          {selectedCity.isIsland === "yes" ? t("common.yes") : t("common.no")}
                        </Label>
                      </div>
                    </div>
                  </div>
                ...
                <div className="flex justify-between">
                  ...
                  >
                    ...
                    {t("cities.managePOI", { count: getPointsForCity(selectedCity.id).length })}
                  </Button>
                  <Button
                    variant="destructive"
                    size="sm"
      ...
      <AlertDialog open={deleteCityId !== null} onOpenChange={() => setDeleteCityId(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t("cities.deleteCityTitle")}</AlertDialogTitle>
            <AlertDialogDescription>
              {t("cities.deleteCityDesc")}
              {deleteCityId && getPointsForCity(deleteCityId).length > 0 && (
                <span className="block mt-2 text-destructive">
                  {t("cities.deleteCityWarning", { count: getPointsForCity(deleteCityId).length })}
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
